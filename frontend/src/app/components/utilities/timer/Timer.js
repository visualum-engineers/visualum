...
const CountDownTimer = ({
    ...
    Object.keys(timeLeft).forEach((interval) => {
        if (timeLeft[interval]>=10) timerComponents.push(
                <span key={interval}>
                    {interval==="seconds"? timeLeft[interval]:timeLeft[interval]+":"}
                </span>
            );
        else if(timeLeft[interval] === 0) timerComponents.push(
                <span key={interval}>
                    {interval==="seconds"?"00":"00:"}
                </span>
            );
        else timerComponents.push(
                <span key={interval}>
                    {`0${interval==="seconds"? timeLeft[interval]:timeLeft[interval]+":"}`}
                </span>
            );
    });
