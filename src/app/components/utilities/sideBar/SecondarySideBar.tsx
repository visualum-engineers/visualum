...
const SecondarySideBar = (props: any) => {
  return (
    <>
      {props.exitSideBarBtn && (
        <button
          className={
            `secondary-exit-sidebar-btn` +
        ...
        >
          <FontAwesomeIcon icon={props.sidebarToggle ? faTimes : faBars} />
        </button>
      )}
      ...
      <nav
        ...
        className={
          `d-flex flex-column sidebar-nav secondary-sidebar-nav ` +
      ...
      >
        {props.logo && (
          <div className="secondary-sidebar-header d-flex justify-content-center align-items-center">
            ...
            >
              <img
                className="secondary-sidebar-visualum-logo"
                src={Logo}
                alt="Visualum logo"
              />
            </a>
        ...
        <div className="secondary-sidebar-link-container d-flex flex-column">
          {props.data &&
            props.data.map((navItem: any, index: number) => {
              return (
                <NavItem
                  key={index}
        ...
        <div
          className={
            `secondary-sidebar-footer d-flex flex-column align-items-center` +
            ` justify-content-${props.userProfile ? "start" : "end"} `
          }
        >
          {props.footerData &&
            props.footerData.map((navItem: any, index: number) => {
              return (
                <NavItem
                  key={index}
                  navType={navItem.type}
      ...
      {!props.windowWidth && (
        <button
          className={`sidebar-dark-bg fixed-top ${
            props.sidebarToggle ? "sidebar-bg-show" : "sidebar-bg-hide"
          }`}
          aria-label="exit-sidebar"
          onClick={props.handleSideBar}
          aria-hidden={!props.sidebarToggle}
