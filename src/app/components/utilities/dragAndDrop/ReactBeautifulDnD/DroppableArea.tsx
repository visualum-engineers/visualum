...
const getRenderItem =
  (content: any, draggableClassName: any, isDraggingClass: any) =>
    ...
    (
      <li
        ...
        className={`${draggableClassName}${
          snapshot.isDragging ? " " + isDraggingClass : ""
        }`}
      >
        <div style={{ textAlign: "center" }}>
          {content[rubric.source.index].content}
        </div>
      </li>
    );
...
const DroppableArea = ({
  ...
  const renderItems = getRenderItem(
    content,
    draggableClassName,
    isDraggingClass
  );
  const InnerList = React.memo(
    ({ content, onTap, removedEl, firstElTap = null }: any) =>
        ...
        return (
          <DraggableItems
            ...
            draggableClassName={
              ...
              `${last ? " last-item" : ""}` +
              `${
                removedEl && removedEl.id === draggableContent.id
                  ? " hide-draggable"
                  : ""
              }` +
              `${
                firstElTap && firstElTap.draggableId === draggableContent.id
                  ? ` ${isDraggingClass}`
                  : ""
              }`
            }
