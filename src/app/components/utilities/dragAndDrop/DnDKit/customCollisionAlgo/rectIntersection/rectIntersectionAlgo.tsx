...
export const rectIntersection = (
  { droppableContainers, collisionRect }: any,
  { containers, overlayRect, isOver, cleanup = false }: any
) => {
  ...
  for (let droppableContainer of droppableContainers) {
    ...
    if (id in containers) {
      ...
      if (exposedNodes.customParentNode)
        currentDroppablePostion = updateDroppableRect(
          exposedNodes.customParentNode
        );
      if (exposedNodes.parentNode)
