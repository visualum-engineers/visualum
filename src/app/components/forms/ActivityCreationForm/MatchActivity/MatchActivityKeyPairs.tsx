...
const MatchActivityKeyPairs = ({
  ...
  return (
    <div className="match-creation-single-key-pair">
      <div className="d-flex h-100 col-10 col-sm-11">
        <div className="match-creation-key col-6 col-sm-5 col-lg-4">
          {!keyEditActive ? (
            <button onDoubleClick={() => setKeyEditActive(true)}>
              {data.key.content}
            </button>
          ) : (
            <div>
        ...
        <div className="match-creation-answer col-6 col-sm-5 col-lg-4">
          {!answerEditActive ? (
            <button onDoubleClick={() => setAnswerEditActive(true)}>
              {data.answer.content}
            </button>
          ) : (
            <div>
      ...
      <div className="match-creation-key-pair-btns col-2 col-sm-1">
        <button
          onClick={
            !keyEditActive && !answerEditActive
              ? () => {
                  unstable_batchedUpdates(() => {
        ...
        >
          <FontAwesomeIcon
            icon={!keyEditActive && !answerEditActive ? faEdit : faSave}
          />
        </button>
        <button onClick={onDeleteKeyPair}>
