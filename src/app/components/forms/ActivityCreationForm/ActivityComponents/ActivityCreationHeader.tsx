...
const ActivityQuestionHeader = ({
  ...
  const [textareaHeight, setAreaHeight] = useState<null|string>(null);
  const storedResize = useRef<null | string>(null);
  const pointsActiveMaxChar = 3;
  const instructionsMaxChar = 400;

  const [pointValue, setPointValue] = useReduxDebouncedTextInputs({
    selectorFunc: (state: RootState) =>
      ...
      <div className="activity-creation-question-instructions">
        {instructionsActive ? (
          <textarea
            style={{
              height:
                textareaHeight && storedResize.current ? textareaHeight : "",
            }}
            ...
            onBlur={(e) => {
              if (e.target.style.height)
                storedResize.current = e.target.style.height;
