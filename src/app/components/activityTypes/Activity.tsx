...
const Activity = () => {
  ...
  const activityData = useSelector(
    (state: RootState) => state.activities.data.originalData.activityData
  );
  const resetPopUp: any = useSelector(
    (state: RootState) => state.activities.settings.resetPopUp
  );
  const userSetDnDEnabled = useSelector(
    (state: RootState) => state.activities.settings.userSetDnDEnabled
  );
  const timeRemindersEnabled = useSelector(
    (state: RootState) => state.activities.settings.timeRemindersEnabled
  );
  ...
  const timerEndTime = useSelector(
    (state: RootState) =>
      state.activities.data.clientData.present.clientAnswerData.activityEndTime
  );
  const timerData =
    timerStartTime && timerEndTime
      ? convertTimeDiff(timerStartTime, timerEndTime)
      : null;
  useEffect(() => {
  ...
  const questionNumber = useSelector(
    (state: RootState) =>
      state.activities.data.clientData.present.clientAnswerData.lastQuestionSeen
  );
  ...
  const handleSideBar = (e: any) => {
    if (
      sidebarToggle &&
      e.target.closest("button").ariaLabel === "exit-sidebar"
    )
      return exitSideBar();
    else return openSideBar();
  };
  //use for activity instructions popup
  const moreInfoOnClick = (e: any) => {
  ...
  return (
    <>
      ...
      <div
        className={`${
          sidebarToggle && mediumWindowWidth ? "secondary-sidebar-open " : ""
        }activity-body`}
      >
        {moreInfoBtn && (
          <ActivityInstructions
        ...
        {timeRemindersEnabled && (
          <ActivityTimeReminder popUpBgStyles={popUpBgStyles} />
        )}
        ...
        <div
          ...
          style={inProp ? { overflow: "hidden" } : {}}
        >
          <div className="activity-header ">
            {timerData && !smallWindowWidth && (
              <div className={`activity-timer`}>
                <span>TIME:</span>
                <Timer timer={timerData} autoStart={true} />
              </div>
            )}
          </div>
          ...
          {question.type ? (
            activityData.questions.map((question, index) => {
              ...
              return (
                <CSSTransition
                  key={`question-${index}`}
                  in={questionNum === index}
                  timeout={duration}
                  classNames={`${
                    moveLeft ? "question-move-left" : "question-move-right"
                  }`}
                  mountOnEnter
                  unmountOnExit
                >
