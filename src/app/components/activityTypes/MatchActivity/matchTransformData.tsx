const transformData = (data: any, itemBankColumns: any) => {
  let newData: any = {};
  ...
  if (!data.itemBank) {
    ...
    for (let i = 0; i < itemBankColumns; i++) {
      const elementsPresent =
        data.answerChoices.length % itemBankColumns === 0
          ? data.answerChoices.length / itemBankColumns
          : Math.floor(data.answerChoices.length / itemBankColumns + 1);
      const startSlice = i * elementsPresent;
      const endSlice = (i + 1) * elementsPresent;
      newData.itemBank["answerChoices-" + i] = data.answerChoices
  ...
  //when data was already transformed on mount
    ...
    for (let i = 0; i < itemBankColumns; i++) {
      ...
      const elementsPresent =
        keys.length % itemBankColumns === 0
          ? keys.length / itemBankColumns
          : Math.floor(keys.length / itemBankColumns + 1);
      const startSlice = i * elementsPresent;
      const endSlice = (i + 1) * elementsPresent;
      newData["itemBank"]["answerChoices-" + i] = keys
        .slice(startSlice, endSlice)
        .map((answer) => {
