...
export default function SettingsModal({ toggle: toggleThis }: any) {
  ...
  const deactivateText =
    "Upon deletion, account can be recovered for up to 30 days. After that, all your data will be removed permanently.";

  const [currentTab, setCurrentTab] = useState("General");
  function handleClickTab(tabName: any) {
  ...
  return ReactDOM.createPortal(
    <ModalContainer
      title="Settings"
      toggle={toggleThis}
      extraClasses={["settings-modal-container-override"]}
    >
        ...
        <div className="settings-modal">
          <div className="body">
            <div className="tabs">
              {tabs.map((tab) => (
                <div
                  className={`tab ${tab.title === currentTab ? "active" : ""}`}
